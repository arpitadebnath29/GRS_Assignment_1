...
python3 MT25018_Part_D_plot.py    # Generate visualizations
...
| **I/O** | 3-5s | 50MB writes (10×5MB blocks) | 5-20% | Low |
...
**Tools**: GCC, pthread, `ps`, `top`, `iostat`, `taskset`, `bc`, Python 3
...
### Worker Details
- **CPU**: Trigonometry, primes, factorials, matrix ops (7 layers)
- **Memory**: Large array operations, random access, cache stress
- **I/O**: File creation, fsync, 4KB blocks, cleanup in `/tmp/`

### Measurement Scripts
- **Part C**: Tests 6 combinations (A/B × cpu/mem/io) with 2 workers
- **Part D**: Variable workers (fork: 2-5, threads: 2-8), generates plots
- **Metrics**: CPU%, Memory, I/O via `top`, `ps`, `iostat`
...
**AI Contributions**:
- Worker function algorithms (partial)
- Bash monitoring scripts (substantial)
- Python plotting code (complete)
- Makefile structure (partial)

**Manual Work**: Core logic, integration, testing, parameter tuning, analysis

All AI-generated code has been reviewed, understood, tested, and integrated. Ready for viva explanation.
...
**Author**: MT25018 | See analysis files for detailed results and insights.
